...
const icon = extend("icon", abstractPhoneConditional).texture(
  "('textures/ui/phud/' + $name)"
);

// Jeb icon wrapper extending abstract base
const jebIconWrapper = extend("jeb_icon_wrapper", abstractPhoneConditional)
...
const backgrounds = panel("backgrounds").controls(
  extend("phone_background", abstractPhoneConditional)
    .texture("textures/ui/phud/box_small")
    .variable("$condition", "((#value = 'ring') or (#value = 'standby'))")
);

const jebBackground = extend("jeb_background", abstractPhoneConditional)
