...
export default defineUI(
  ...
  (ns) => {
    ...
    // Add all elements to namespace - dataControl needs to be added for reference
    ns.add(dataControl).add(renderers).add(elements);

    // Main container panel
    ns.add(panel("main").fullSize().controls(renderers, elements));
  },
  { subdir: "phud" }
);
